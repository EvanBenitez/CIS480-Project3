...
//Program counter
int PC = 508; // start behind to compensate for first pc increment
...
void init(char *fileName){
  ...
  for(int i = 0; i < 256 && fread(&raw,4,1,file); i++){
    memory[i] = raw; //convert to little endian order
  }
...
void print_result(){
  ...
  //print pipeline registers (instructions and control signal in Hexidecimal)
  printf("IF/ID (pc4, inst)                               %-4x %08x\n", IF_ID.pc4, IF_ID.inst);
  printf("ID/EX (pc4, rd1, rd2, extend, rt, rd, ctrl)     %-4x %-4x %-4x %-4x %-4x %-4x %03x\n", ID_EX.pc4, ID_EX.rd1, ID_EX.rd2, ID_EX.extend, ID_EX.rt, ID_EX.rd, ID_EX.ctrl);
  printf("EX/MEM (btgt, zero, ALUOut, rd2, RegRd, ctrl)   %-4x %-4x %-4x %-4x %-4x %03x\n", EX_MEM.btgt, EX_MEM.zero, EX_MEM.ALUOut, EX_MEM.rd2, EX_MEM.RegRd, EX_MEM.ctrl);
  printf("MEM/WB (memout, ALUOut, RegRd, ctrl)            %-4x %-4x %-4x %03x\n", MEM_WB.memout, MEM_WB.ALUOut, MEM_WB.RegRd, MEM_WB.ctrl);
...
int IF_Stage(){
  ...
  if(!halt){
    ...
    count_down++;
    IF_ID.inst = 0; //no more instructions
  }
...
void ID_Stage(){
  ...
  if(IF_ID.inst != 0 && IF_ID.inst != 12){
    ...
    ID_EX.ctrl = 0x588; //R-type control signal
  }
}
...
void MEM_Stage(){
  ...
  if( (((EX_MEM.ctrl >> 4) & 1) && EX_MEM.zero == 1) //Branch if equal
   || (((EX_MEM.ctrl >> 1) & 1) && EX_MEM.zero == 0) ){ //Branch if not equal
        PC_delay = EX_MEM.btgt-4; //minus 4 to conpensate for PC udate at the beginning of IF_Stage
  }
...
void Branch_Update(){
  if( PC_delay != -1){
    PC = PC_delay;
    IF_ID.inst = 32; //NOP Instruction (add $0,$0,$0)
